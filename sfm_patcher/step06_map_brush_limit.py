# -*- coding: utf-8 -*-
"""
Step 6 — BSP brush count at map load (engine.dll).

Engine error: "Map has too many brushes"

Raises the runtime check in CMod_LoadBrushes from 8192 (0x2000) to 16384 (0x4000).
Does not change VBSP compile limits or brushside/vert caps.
"""
...
from lib.patch_defs import MAP_BRUSH_PATCHES  # noqa: E402

TARGET = "engine.dll"
ERROR_STRING = b"Map has too many brushes"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="SFM Step 6: Map brush limit 8192 -> 16384 (engine.dll)"
    )
    ...
    planned = plan_signature_patches(data, MAP_BRUSH_PATCHES)
    if not planned:
    ...
    log("Done: brush limit 8192 -> 16384. Restart SFM.")
